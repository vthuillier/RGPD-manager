...
class RightsExerciseRepository
{
    ...
    {
        if ($exercise->id === null) {
            $stmt = $this->pdo->prepare('INSERT INTO rights_exercises (user_id, organization_id, applicant_name, request_date, request_type, status, completion_date, details)
                 VALUES (:user_id, :organization_id, :applicant_name, :request_date, :request_type, :status, :completion_date, :details)');
        } else {
            $stmt = $this->pdo->prepare('UPDATE rights_exercises SET applicant_name = :applicant_name, request_date = :request_date, 
