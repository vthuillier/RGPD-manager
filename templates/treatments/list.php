<div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8 gap-4">
    ...
    <div class="flex gap-3">
        ...
        <div class="flex border border-slate-200 rounded-md shadow-sm overflow-hidden bg-white">
            <a href="index.php?page=treatment&action=export_csv"
                class="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 border-r border-slate-200"
                title="Exporter en CSV">
                CSV
            </a>
            <a href="index.php?page=treatment&action=export_pdf" target="_blank"
                class="px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" title="Imprimer / PDF">
                PDF
            </a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nom du
                            traitement</th>
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    <?php foreach ($treatments as $treatment): ?>
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    ...
                                    <?php if ($treatment->hasSensitiveData && $treatment->isLargeScale): ?>
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200"
                                            title="AIPD fortement conseillée">
                                            AIPD
                                        </span>
                                    <?php endif; ?>
                                </div>
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="text-sm text-slate-600 line-clamp-1"><?= htmlspecialchars($treatment->purpose) ?></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <a href="index.php?page=treatment&action=edit&id=<?= $treatment->id ?>"
